import type { MessageTree } from "../types";

export const zhHant: MessageTree = {
  nav: {
    home: "首頁",
    accounts: "帳戶",
    ledger: "記帳",
    insights: "分析",
  },
  loading: "正在載入你的資產…",
  common: {
    cancel: "取消",
    save: "儲存",
    saveChanges: "儲存變更",
    add: "新增",
    close: "關閉",
    dialog: "對話框",
    required: "此欄位必填",
    invalidAmount: "請輸入有效金額（0 或以上）",
    invalidDate: "請輸入有效日期",
  },
  error: {
    title: "發生錯誤",
    message: "出現意外錯誤。你的資料仍安全儲存在本機。",
    retry: "重試",
  },
  storage: {
    nearFull: "瀏覽器儲存空間快滿。請盡快到設定匯出 JSON 備份，以免遺失資料。",
  },
  home: {
    greetingMorning: "早安",
    greetingAfternoon: "午安",
    greetingEvening: "晚安",
    netWorthProfile: "淨資產概覽",
    assets: "資產",
    liabilities: "負債",
    chartEmptyNoAccounts: "新增帳戶以查看淨資產趨勢。",
    chartEmptyNeedData: "更新結餘或建立快照以生成圖表。",
    settings: "設定",
    filter: "篩選",
    addAccount: "新增帳戶",
    moreOptions: "更多選項",
    accountsSection: "帳戶",
    viewAll: "查看全部",
    noAccounts: "尚未有帳戶",
    addFirstAccount: "新增第一個帳戶",
    filteredNetWorth: "篩選後淨資產",
    filteredBadge: "已篩選",
  },
  appLock: {
    title: "WorthBook 已鎖定",
    unlock: "以生物辨識解鎖",
    failed: "驗證失敗，請再試一次。",
  },
  accountForm: {
    addTitle: "新增帳戶",
    editTitle: "編輯帳戶",
    asset: "資產",
    liability: "負債",
    name: "名稱",
    namePlaceholder: "例如：匯豐儲蓄",
    category: "分類",
    currency: "貨幣",
    currentValue: "目前結餘",
    asOfDate: "截至日期",
    asOfHint: "此結餘對應的日期；儲存後會更新該日的淨資產紀錄。",
    institution: "機構",
    note: "備註",
    optional: "選填",
    nameRequired: "請輸入帳戶名稱",
    valueRequired: "請輸入有效結餘",
    valueSignHint: "資產可填負數（透支）。負數資產會自動存成負債。",
    liabilityValueHint: "請輸入尚欠金額（正數即可，唔使填負號）。",
  },
  valueForm: {
    addTitle: "更新結餘",
    editTitle: "編輯結餘",
    amountRequired: "請輸入有效金額",
    amountSignHint: "撳 + / − 設定正數或負數結餘。",
    liabilityAmountHint: "請輸入尚欠金額（正數即可，唔使填負號）。",
  },
  txForm: {
    addTitle: "新增記帳",
    editTitle: "編輯記帳",
    amountRequired: "請輸入大於零的金額",
    titleRequired: "請輸入標題",
  },
  intro: {
    skip: "略過",
    next: "下一步",
    back: "上一步",
    welcome: {
      title: "我應該點樣稱呼你？",
      subtitle: "呢個名會顯示喺主頁，之後可以喺設定更改。",
      nameLabel: "你的稱呼",
      namePlaceholder: "例如：阿明",
    },
    asset: {
      title: "加入你嘅第一個資產",
      subtitle: "由你已經有嘅開始 — 現金、銀行結餘，或者投資帳戶。",
      tip1: "揀「資產」（唔係負債）。",
      tip2: "輸入而家嘅金額同貨幣。",
      tip3: "之後隨時可以更新結餘。",
      cta: "加入第一個資產",
      skip: "暫時略過",
    },
    expense: {
      title: "記錄你嘅第一筆支出",
      subtitle: "WorthBook 會一齊追蹤開支同淨資產 — 試下記一筆支出。",
      tip1: "揀類別，例如飲食或交通。",
      tip2: "可以連結到你啱啱加嘅帳戶。",
      tip3: "記帳資料會用嚟產生每週報告。",
      cta: "加入第一筆支出",
      skip: "暫時略過",
    },
    insights: {
      title: "點樣睇 Insights 圖表",
      subtitle: "Insights 分頁會將帳戶同記帳變成圖表 — 增長、現金流、配置等等。",
      tip1: "之後喺底部選單㩒 Insights 就可以入去。",
      tip2: "頂部可以切換圖表類型（增長、現金流、配置…）。",
      tip3: "你愈更新結餘同支出，圖表就會愈齊全。",
      cta: "明白",
      skip: "暫時略過",
    },
    report: {
      title: "睇下報告點樣出",
      subtitle: "每個星期同每個月，WorthBook 會根據你嘅資料整一份 Wrapped 風格回顧。下面係範例。",
      cta: "睇範例報告",
      skip: "完成設定",
    },
    sample: {
      introTitle: "你嘅 WorthBook 回顧",
      introSubtitle: "範例星期 · 範例月份",
      weekHeading: "本週一覽",
      income: "收入",
      expense: "支出",
      net: "淨現金流",
      highlightsHeading: "重點",
      sampleExpenseTitle: "同朋友食飯",
      sampleExpenseCategory: "飲食",
      sampleIncomeTitle: "人工",
      sampleIncomeCategory: "收入",
      monthHeading: "本月淨資產",
      netWorth: "淨資產",
      change: "變化",
      outroTitle: "搞掂啦",
      outroSubtitle: "繼續追蹤 — 新嘅一星期或月份開始時會有真正回顧。",
    },
  },
  settings: {
    eyebrow: "偏好設定",
    title: "設定",
    subtitle: "離線優先。你的結餘唔會離開此瀏覽器。",
    displayName: "顯示名稱",
    displayNameHint: "顯示喺主頁左上角，取代 WorthBook。",
    displayNamePlaceholder: "你的稱呼",
    display: "顯示",
    language: "語言",
    languageEn: "English",
    languageZhHant: "繁體中文",
    languageZhHans: "简体中文",
    theme: "主題",
    themeLight: "淺色",
    themeDark: "深色",
    themeSystem: "跟隨系統",
    themeLightHint: "白底 + 綠色重點",
    themeDarkHint: "深色底 + 亮綠重點",
    themeSystemHint: "跟隨裝置淺色 / 深色模式",
    privacyTitle: "私隱模式",
    privacyDesc: "以圓點遮蓋結餘",
    reportsTitle: "結算回顧",
    reportsDesc: "記帳同淨資產合併一份回顧 — 新週或新月首次打開時自動彈出。",
    viewRecap: "睇回顧",
    weeklyNotifications: "每週提醒",
    weeklyNotificationsDesc: "星期一早上 — 開啟記帳 Wrapped",
    monthlyNotifications: "每月提醒",
    monthlyNotificationsDesc: "每月 1 號 — 淨資產 Wrapped",
    reportsNativeHint: "排程提醒僅限 iOS/Android App。網頁版會喺你打開 WorthBook 時顯示回顧。",
    rateInvalid: "請輸入大於零的匯率",
  },
  streak: {
    sectionEyebrow: "選用",
    sectionTitle: "每日連續紀錄",
    sectionDesc:
      "輕量習慣輔助 — 更新結餘、記帳，或喺呢度簽到。唔會改動你嘅投資組合數字。",
    best: "最長 {n}",
    freezesLeft: "剩餘 {n} 次凍結",
    checkIn: "今日簽到",
    checkedIn: "今日已完成",
    milestones: "里程碑",
    milestoneDays: "{n} 日",
    weekdayMon: "一",
    weekdayTue: "二",
    weekdayWed: "三",
    weekdayThu: "四",
    weekdayFri: "五",
    weekdaySat: "六",
    weekdaySun: "日",
    celebrationDay: "第 {day} 日！",
    celebrationTitle: "連續紀錄已延長",
    celebrationBody: "聽日再嚟延續。",
    celebrationFreezeUsed: "已用凍結補返漏咗嘅一日。",
    celebrationNice: "好",
  },
  reports: {
    emptyTitle: "暫時未夠資料",
    emptyBody: "請先記帳，或更新帳戶價值（至少兩個日期），再試 View Recap。",
    close: "關閉",
    next: "下一步",
    done: "完成",
    tapContinue: "㩒一下繼續",
  },
};
