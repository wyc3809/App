...
export function NetWorthHero() {
  ...
  const positive = (change?.absolute ?? 0) >= 0;
    ...
    <section className="card-surface animate-fade-up overflow-hidden p-5">
      ...
      {change && (
        ...
        >
          ...
          <span>
            {privacy
              ? "••••"
              : `${formatMoney(change.absolute, settings.baseCurrency, currencies, {
                  showSign: true,
                  compact: true,
                })} (${formatPercent(change.percent)})`}
          </span>
          <span className="font-medium opacity-70">all time</span>
        </div>
      )}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <div className="rounded-2xl p-3" style={{ background: "var(--accent-soft)" }}>
          ...
          <p className="mt-1 text-lg font-semibold tabular-nums" style={{ color: "var(--positive)" }}>
        ...
        <div className="rounded-2xl p-3" style={{ background: "var(--danger-soft)" }}>
          ...
          <p className="mt-1 text-lg font-semibold tabular-nums" style={{ color: "var(--negative)" }}>
